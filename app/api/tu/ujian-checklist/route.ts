...
const Body = z.object({
  ...
  checklist: z.array(z.boolean()).length(KUT_CHECKLIST_ITEMS.length),
});

// TU menyimpan ceklis ADA / TIDAK ADA berkas Ujian Tesis mahasiswa.
export async function POST(req: Request) {
  ...
  await prisma.tesis.update({
    where: { id: parsed.tesisId },
    data: { ujianChecklist: parsed.checklist },
  });

  await prisma.auditLog.create({
    data: {
      actorId: session.uid,
      action: "UJIAN_CHECKLIST_UPDATE",
      entity: "Tesis",
