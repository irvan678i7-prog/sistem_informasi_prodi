...
// Mahasiswa mengunggah berkas syarat Ujian Tesis per item check list (14 item).
// Analog dengan /api/tesis/seminar-proposal/upload.
export async function POST(req: Request) {
  ...
  if (
    ...
    item < 1 ||
    item > KUT_CHECKLIST_ITEMS.length ||
  ...
  const path = `ujian-tesis/${session.uid}/item${item}-${Date.now()}-${file.name}`;
  ...
  await prisma.ujianBerkas.upsert({
